...
ChatLogic::~ChatLogic()
{
    ...
    {
        delete *it;
    }*/ // delete because a unique pointer _node has been created.
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));// Change Task 3
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto edge = std::make_unique<GraphEdge>(id); // Change Task 3
    ...
    //std::unique_ptr<GraphNode> rootNode; // Change due to Task 3
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
...
{
    //std::cout<<"ChatLogic Copy Assignment";
    _chatBot = chatbot;
    //std::cout<<".\n";
}
